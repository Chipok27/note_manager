...
# Функция ввода даты с проверкой
def input_date():
    date_valid = False
    date_ = ''
...
def select_menu():
    while True:
        ...
        print('6. Запись Заметок в файл.')
        print('7. Чтение Заметок из файла.')
...
# Процедура записи Заметок в файл
def save_notes_to_file(notes: list, file_name='Notes_Recorded.txt'):
    file = open(file_name, 'w', encoding='utf-8')
    for note in notes:
        # Проверка актуальности Заметки (дата ещё не истекла)
        if datetime.datetime.strptime(note['issue_date'], '%d-%m-%Y') > datetime.datetime.now():
...
# Функция чтения данных из файла и возвращения списка Заметок (словарей)
def load_notes_from_file(file_name='Notes_Recorded.txt'):
    notes = []
    ...
    while text:
        if ':' in text:
            ...
            if key == 'status': # Статус в базе Заметок в виде числового значения
                value = int(value.strip())
            else:
                value = value.strip()

            if key == 'username': # уже не первая заметка
                if note:
                    notes.append(note)
                note = {}
                count_title = 0

            if key == 'title': # добавление заголовков в виде списка
                if count_title == 0:
                    note[key] = []
                count_title += 1
                note[key].append(value)
            else:
                note[key] = value
...
if __name__ == '__main__':
    ...
    while True:
        ...
        elif menu == 6:  # Запись Заметок в файл
            save_notes_to_file(note_list)

        elif menu == 7:  # Чтение Заметок из файла и их вывод на экран

            file_name = input(
                'Введите имя файла или нажмите "ENTER" для имени "Notes_Recorded.txt" : ') or 'Notes_Recorded.txt'
